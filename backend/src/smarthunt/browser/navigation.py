...
class NavigationService:
    ...
    async def goto_job(self, page: Page, url: str) -> None:
    ...
    async def wait_until_loaded(self, page: Page) -> None:
        try:
    ...
    async def verify_job_page(self, page: Page) -> None:
        ...
        for selector in JOB_PAGE_SELECTORS:
            try:
                ...
                logger.exception("Failed checking selector %s", selector)

        raise JobPageNotFound("Could not verify job page.")

    async def get_job_title(self, page: Page) -> str:
        try:
            return await page.title()
        except Exception:
            return ""

    async def open_job(self, page: Page, url: str) -> str:
        await self.goto_job(page, url)
        await self.wait_until_loaded(page)
        await self.verify_job_page(page)

        return await self.get_job_title(page)
